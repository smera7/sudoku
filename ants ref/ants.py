...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current.is_hive:
            if current.bees and self.min_range <= places_away <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if not self.health - amount: #if it doesn't die
            reflective_damage += self.damage
        if self.health <= 0:
            self.place.remove_insect(self) #access by calling place
            self.death_callback()
        for i in list(self.place.bees): 
            i.reduce_health(reflective_damage) #don't add self when you call an instance
        Ant.reduce_health(self, amount) #Ant health reduced too
...
#         """Create an Ant with a HEALTH quantity."""
#         super().__init__(health)
#         self.chew_countdown = 0 #instance attribute with inheritance
...
#             if bees:
#                 chosen = random_bee(bees)
#                 Insect.reduce_health(chosen, chosen.health)
#             self.chew_countdown = 3 #assigning to 3 just doesn't work

class HungryAnt(Ant):
    """HungryAnt eats a random bee in its place and chews for 3 turns."""
    ...
    def action(self, gamestate):
        """Picks random bee from place and reduces its health to 0, does nothing if chewing."""
        if self.chew_countdown:
            self.chew_countdown -= 1
        else:
            bees = self.place.bees
